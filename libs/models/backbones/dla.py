...
class LaneBasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1,):
        super(LaneBasicBlock, self).__init__()
    ...
    def forward(self, x, residual=None):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        # print(f"After conv1 and bn1: {out.shape}")
        out = self.relu(out)
        
        out = self.conv2(out)
        # out = self.adjust_channels(out)  # 调整通道数

...
class CLRerHead(nn.Module):
    ...
    def loss(self, out_dict, img_metas):
        ...
        DLiou_loss = torch.tensor(0.0).to(device)
        DRiou_loss = torch.tensor(0.0).to(device)
        lane_iou = torch.tensor(0.0).to(device)
        r_iou = torch.tensor(0.0).to(device)
        l_iou = torch.tensor(0.0).to(device)
        ...
        for stage in range(self.refine_layers):
            for b, img_meta in enumerate(img_metas):
                ...
                lane_iou, r_iou, l_iou = self.loss_iou(
                    pred_xs * (self.img_w - 1) / self.img_w,
                    target_xs / self.img_w,
                    img_meta["eval_shape"],
                )
                
                iou_loss=iou_loss + lane_iou
                DLiou_loss=DLiou_loss + r_iou
                DRiou_loss=DRiou_loss + l_iou
        ...
        DLiou_loss /= batch_size * self.refine_layers
        DRiou_loss /= batch_size * self.refine_layers
